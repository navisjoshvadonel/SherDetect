...
export const DomainBanner: React.FC<DomainBannerProps> = ({
  ...
  return (
    <section className="neo-card p-4 bg-white">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-3 border-b-2.5 border-brutal-black pb-2.5">
        <div className="flex items-center gap-2">
          ...
          {activeDomainFilter !== "all" ? (
            ...
            <span className="text-[10px] font-black bg-brutal-bg text-slate-700 px-2 py-0.5 border border-brutal-black rounded">
              ALL DOMAINS ACTIVE
            </span>
          )}
        </div>
        ...
        {domains.map((d) => {
          ...
          return (
            <button
              ...
              className={`p-2.5 ${d.bgColor} border-2 border-brutal-black rounded-xl text-center shadow-brutal-sm transition-all duration-200 cursor-pointer ${
                isSelected
            ...
            >
              <i className={`${d.icon} text-lg text-brutal-black mb-1 block transition-transform group-hover:scale-110`}></i>
              <span className="text-[11px] font-black uppercase text-brutal-black block truncate">
                {DOMAIN_LABELS[d.key]}
              </span>
              <span className="text-[9.5px] font-bold text-slate-700 block truncate mt-0.5">
                {d.description}
              </span>
            </button>
          );
